...
func (c *config) confDirLiteral() string {
	return strings.TrimPrefix(c.confDir(), "/host")
}
...
func run(ctx context.Context) error {
	...
	if err := writeNodeName(cfg); err != nil {
		return fmt.Errorf("writing node name: %w", err)
	}
...
func writeWhereaboutsConf(cfg *config) error {
	conf := whereaboutsConf{
		...
		ConfigurationPath: cfg.confDirLiteral(),
...
func writeNodeName(cfg *config) error {
	if cfg.NodeName == "" {
		slog.Warn("NODENAME not set; skipping nodename file generation")
		return nil
	}
	if err := os.WriteFile(cfg.nodeNamePath(), []byte(cfg.NodeName), defaultKubeConfigMode); err != nil {
		return fmt.Errorf("writing nodename to %s: %w", cfg.nodeNamePath(), err)
	}
	slog.Info("wrote nodename", "path", cfg.nodeNamePath())
	return nil
}
