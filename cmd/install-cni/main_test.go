...
func TestLoadConfig_Defaults(t *testing.T) {
	...
	assertEqual(t, "NodeName", cfg.NodeName, "")
...
func TestLoadConfig_CustomValues(t *testing.T) {
	...
	t.Setenv("NODENAME", "worker.example.com")
	...
	assertEqual(t, "NodeName", cfg.NodeName, "worker.example.com")
	if !cfg.SkipTLSVerify {
...
func TestConfigPaths(t *testing.T) {
	...
	assertEqual(t, "confDirLiteral", c.confDirLiteral(), "/etc/cni/net.d/whereabouts.d")
}
...
func TestWriteWhereaboutsConf(t *testing.T) {
	...
	assertEqual(t, "configuration path", parsed.ConfigurationPath, cfg.confDirLiteral())
...
func TestWriteNodeName(t *testing.T) {
	cfg := newTestConfig(t, false)
	must(t, writeNodeName(cfg))

	data, err := os.ReadFile(cfg.nodeNamePath())
	must(t, err)
	assertEqual(t, "nodename", string(data), cfg.NodeName)
...
func TestWriteNodeName_Empty(t *testing.T) {
	cfg := newTestConfig(t, false)
	cfg.NodeName = ""
	must(t, writeNodeName(cfg))

	if _, err := os.Stat(cfg.nodeNamePath()); !os.IsNotExist(err) {
		t.Fatalf("nodename file should not exist, got error %v", err)
	}
}
...
func TestRun_FullSuccess(t *testing.T) {
	...
	} else if string(nodeName) != cfg.NodeName {
		t.Errorf("nodename = %q, want %q", string(nodeName), cfg.NodeName)
	}
