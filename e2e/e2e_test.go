...
var _ = Describe("Whereabouts functionality", func() {
	...
	Context("Operator conformance", func() {
		const (
			operatorDeploymentName = "whereabouts-operator"
			operatorNamespace      = "kube-system"
			operatorReadyTimeout   = 60 * time.Second
		)
		...
		It("verifies that the whereabouts operator deployment is available", func() {
			By("waiting for the whereabouts operator deployment to have an available replica")
			Expect(wbtestclient.WaitForDeploymentAvailable(
				context.TODO(),
				clientInfo.Client,
				operatorNamespace,
				operatorDeploymentName,
				operatorReadyTimeout,
			)).To(Succeed())

			By("verifying the whereabouts operator pod is running")
			podList, err := wbtestclient.ListPods(
				context.TODO(),
				clientInfo.Client,
				operatorNamespace,
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(podList.Items).NotTo(BeEmpty(), "expected at least one whereabouts operator pod")
